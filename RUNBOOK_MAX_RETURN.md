...
| **HMM regime overlay** | 3-state Gaussian HMM on trailing 3yr SPY returns. Scales gross by regime (BULL 1.00 / NEUTRAL 0.85 / BEAR 0.65). Only acts when posterior confidence > 55%. **Tri-state env**: `0`=off / `SHADOW`=log only / `1`=live. Inspired by Hamilton 1989 + MDPI 2020. | `REGIME_OVERLAY_ENABLED=0` | `=SHADOW` (validate first), then `=1` | Tail-loss reduction in confirmed bears; near-zero effect in ambiguous regimes |
